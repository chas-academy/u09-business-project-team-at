...
export default function CreateAListModal() {
    ...
    const handleOnSubmit = async (e: React.FormEvent) => {
        ...
        const result = await ListService.createList(
            token,
            user.id,
            ((e.target as HTMLFormElement).elements.namedItem('name') as HTMLInputElement)?.value
        );
    ...
    return (<>
        <BaseModal open={true} onClose={() => invokeCreateAListModal(false)} title="Create a new list">
            <form className="flex flex-col w-full  gap-1 text-white text-base font-bold text-left" onSubmit={handleOnSubmit}>
                ...
                <div className="flex gap-1 mt-4">
                    <Button 
                        className="py-4 border-white border-2" 
                        variant="primary"
